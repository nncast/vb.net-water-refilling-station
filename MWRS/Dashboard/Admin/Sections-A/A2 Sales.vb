Public Class A2_Sales
    ...
    Private Sub btnupdate_Click(sender As Object, e As EventArgs) Handles btnupdate.Click
        Try
            ' --- Validate ---
            If lvorderitems.Items.Count = 0 Then
                MsgBox("No items to update.", MsgBoxStyle.Information, "Validation Error")
                Exit Sub
            End If

            Dim orderId As Integer = CInt(lblorderid.Text)
            Dim custid As Integer = CInt(cmbcustomer.SelectedValue)
            Dim newTotal As Decimal = CDec(lbltotalprice.Text)
            Dim userid As Integer = 1 ' Replace with actual logged-in user

            ' --- Confirm Update ---
            If MsgBox("Are you sure you want to update this order?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Confirm Update") = MsgBoxResult.No Then
                Exit Sub
            End If

            ' --- Step 1: Restore stock for previous products ---
            GetQuery("SELECT productid, qty FROM tblorderitems WHERE orderid = " & orderId & " AND itemtype='Product'", "oldProducts")
            For Each row As DataRow In ds.Tables("oldProducts").Rows
                Dim pid As Integer = CInt(row("productid"))
                Dim qtyOld As Integer = CInt(row("qty"))
                SetQuery("UPDATE tblproducts SET stockqty = stockqty + " & qtyOld & " WHERE productid = " & pid)
            Next

            ' --- Step 2: Delete previous order items ---
            ...
            For Each item As ListViewItem In lvorderitems.Items
                ...
                If itemType = "Product" Then
                    ...
                    ' Insert updated product
                    SetQuery("INSERT INTO tblorderitems (orderid, productid, qty, price, itemtype) " &
                             "VALUES (" & orderId & ", " & productid & ", " & qty & ", " & price & ", 'Product')")
                    ...
                    ' Log inventory transaction
                    SetQuery("INSERT INTO tblinventorytransactions (productid, userid, transtype, qty, remarks) " &
                             "VALUES (" & productid & ", " & userid & ", 'Stock Out', " & qty & ", 'Updated Order #" & orderId & "')")

                ElseIf itemType = "Service" Then
                    Dim serviceid As Integer = CInt(item.SubItems(6).Text)

                    ' Insert updated service
                    SetQuery("INSERT INTO tblorderitems (orderid, serviceid, qty, price, itemtype) " &
                             "VALUES (" & orderId & ", " & serviceid & ", " & qty & ", " & price & ", 'Service')")
                End If
            Next

            ' --- Step 4: Update tblorders ---
            Dim orderType As String = cmbordertype.Text
            Dim orderDate As String = dtpdate.Value.ToString("yyyy-MM-dd HH:mm:ss")
            SetQuery("UPDATE tblorders SET ordertype = '" & orderType & "', orderdate = '" & orderDate & "' WHERE orderid = " & orderId)

            ' --- Step 5: Update tblsales and customer balance ---
            GetQuery("SELECT totalamount FROM tblsales WHERE orderid = " & orderId, "oldSale")
            Dim oldTotal As Decimal = 0
            If ds.Tables("oldSale").Rows.Count > 0 Then
                oldTotal = CDec(ds.Tables("oldSale").Rows(0)("totalamount"))
            End If

            ' Update sales total
            SetQuery("UPDATE tblsales SET totalamount = " & newTotal & ", saledate = NOW() WHERE orderid = " & orderId)

            ' Adjust customer balance
            Dim diff As Decimal = newTotal - oldTotal
            SetQuery("UPDATE tblcustomerbalance SET balance = balance + " & diff & ", lastupdate = NOW() WHERE custid = " & custid)
            LogActivity("Sales", "Update Order", orderId)

            ' --- Step 6: Feedback and reset form ---
            MsgBox("Order #" & orderId & " updated successfully!", MsgBoxStyle.Information, "Update Complete")

            ' Reset form to new order state
            SetSalesFormState("new")
            LoadProducts()
            LoadNextOrderID()
            lvorderitems.Items.Clear()

            ' Refresh orders list in A_Orders
            A3_Orders.FillOrders()

        Catch ex As Exception
            MsgBox("An error occurred while updating the order: " & ex.Message, MsgBoxStyle.Critical, "Error")
        End Try
    End Sub
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        ...
        ' Open Orders form
        labelclickedA(AdminDashboard.lblorders)
        AdminDashboard.switchPanel(A3_Orders)
        AdminDashboard.lbltitle.Text = "Orders"
        A3_Orders.FillOrders()
    End Sub
